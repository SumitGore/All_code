...
public class demo3 {
    ...
    public static void main(String[] args) {
        String filePath = "D:\\CRF _Customer_CalculatedTags1.xlsx";
        
        try {
            // Read existing Excel file
            FileInputStream fileInputStream = new FileInputStream(filePath);
            Workbook workbook = new XSSFWorkbook(fileInputStream);
           // Workbook newWorkbook = new XSSFWorkbook();   //new workbook create
            // Get the first sheet
            ...
            for (Map.Entry<String, Map<List<String>,List<String>>> entry : customerTagMap.entrySet()) {
                ...
                Map<List<String>,List<String>> tagsEntry = entry.getValue();
                 ...
                 for (Map.Entry<List<String>,List<String>> tagValues : tagsEntry.entrySet()) {
				    ...
				    Sheet customerSheet = newWorkbook.getSheet(modifiedCustomerName);

				 // If the sheet already exists, you can skip or handle it as needed
				 if (customerSheet != null) {
				     System.out.println("Sheet '" + modifiedCustomerName + "' already exists. Skipping.");
				     continue;  // Skip creating a new sheet
				 }

				 // Create a new sheet with the modified name for each customer
				 customerSheet = newWorkbook.createSheet(modifiedCustomerName);
				    ...
				    try {
				        ...
				        if (!"na".equals(custId)) {
				            ...
				            List<String> deviceName = printJsonNode3(objectMapper.readTree(devices), "name");
				                ...
				                // Iterate through devices and print values in the new sheet
					            int rowIndex = 0;
					            // Create header row in the new sheet
					            Row header = customerSheet.createRow(rowIndex);
					            
					         // Create a header row for the new sheet outside the loop
					            ...
					            // Iterate over devices and append values to the new sheet
					            for (int i = 0; i < deviceIds.size(); i++) {
					                ...
					                // Set device ID and tag values in the new row
...
////						            // Set device ID and tag values in the new row
//				                    header1.createCell(0).setCellValue(start.toString());    // 1st column date, value set 
//				                    header1.createCell(1).setCellValue(deviceName.get(i).replaceAll("\"", ""));  // Device Name set 2nd column values
//				                    addToExcel(2, header1, tagValuesAPI);    // next column set tags and keys values
//				                   // nxtRow++;
					             }
         ...
         // Save the changes back to the Excel file
            FileOutputStream fileOutputStream = new FileOutputStream("D:\\CRF _Customer_TagsValues11.xlsx");
    ...
    private static void addToExcel(int startIdx, Row headerRow, List<Double> values) {
        int idx = startIdx;
