            ...
            if (titleRow != null) {
                ...
                if (titleCellInColumn5 == null || titleCellInColumn5.getCellTypeEnum() == CellType.BLANK) {
                    ...
                    for (String sheetName : uniqueValues) {
                        try {
                            // Check if column 5 is empty for the title row
                            if (titleRow.getCell(4) == null || titleRow.getCell(4).getCellTypeEnum() == CellType.BLANK) {
                                System.out.println("Column 5 is empty in the title row. Skipping sheet creation.");
                                continue;  // Skip creating sheets if column 5 is empty in the title row
                            }
                            // Check if column 5 is empty for the existing sheet
                            int sheetIndex = getSheetIndex(workbook, sheetName);
                            if (sheetIndex != -1) {
                                Sheet existingSheet = workbook.getSheetAt(sheetIndex);
                                Cell titleCellInExistingSheet = existingSheet.getRow(2).getCell(4); // Assuming the specified column index is 4 (0-based)
                                if (titleCellInExistingSheet == null || titleCellInExistingSheet.getCellTypeEnum() == CellType.BLANK) {
                                    System.out.println("Column 5 is empty in the existing sheet. Skipping sheet creation.");
                                    continue;  // Skip creating sheets if column 5 is empty in the existing sheet
                                }
                            }

                            // Continue with creating the sheet and copying rows...
                            Sheet newSheet;
                            int newSheetIndex = getSheetIndex(workbook, sheetName);
                            ...
                            if (newSheetIndex == -1) {
                                ...
                                System.out.println("Sheet with name '" + sheetName + "' already exists at index " + newSheetIndex);
                                // Get the existing sheet if it already exists
                                newSheet = workbook.getSheetAt(newSheetIndex);
                            }
